...
def generate_target_pages(all_items):
    ...
    for target in TARGETS:
        ...
        html = build_category_page(
            title=f"{target} 지원금 2026 총정리",
            description=f"2026년 {target} 정부지원금 종류와 신청방법을 총정리했습니다.",
            items=items,
...
def generate_field_pages(all_items):
    ...
    for field in FIELDS:
        ...
        html = build_category_page(
            title=f"{field} 지원금 2026 총정리",
            description=f"2026년 {field} 정부지원금 종류와 신청방법을 총정리했습니다.",
            items=items,
            category_type="field",
            category_name=field,
            filters=TARGETS,
...
<div style="background:linear-gradient(135deg,#FFF7ED,#FFEDD5);padding:36px 20px;text-align:center;border-bottom:1px solid #FFE4C4;">
  <div style="font-size:13px;font-weight:600;color:var(--primary);margin-bottom:8px;">2026 최신 정보</div>
...
<footer class="footer">
  <div class="footer-inner">
    ...
    <div class="footer-bottom"><p>© 2026 bojopass</p></div>
  </div>
</footer>
<script>
...
def generate_sitemap(all_items):
    ...
    urls = [
        '<url><loc>https://bojopass.kr/</loc><changefreq>daily</changefreq><priority>1.0</priority></url>',
        '<url><loc>https://bojopass.kr/list.html</loc><changefreq>daily</changefreq><priority>0.9</priority></url>',
        '<url><loc>https://bojopass.kr/calendar.html</loc><changefreq>weekly</changefreq><priority>0.7</priority></url>',
    ]
    for t in TARGETS:
        urls.append(f'<url><loc>https://bojopass.kr/대상/{t}.html</loc><changefreq>weekly</changefreq><priority>0.8</priority></url>')
    for f in FIELDS:
        urls.append(f'<url><loc>https://bojopass.kr/분야/{f}.html</loc><changefreq>weekly</changefreq><priority>0.8</priority></url>')
    for item in all_items:
        if not item.get("is_closed"):
            urls.append(f'<url><loc>https://bojopass.kr/jiwon/{item["slug"]}.html</loc><lastmod>{today}</lastmod><changefreq>weekly</changefreq><priority>0.7</priority></url>')
